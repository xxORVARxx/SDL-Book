...
{
  m_delta_time = the_World::Instance().Get_delta_time();
  m_mouse_pos = the_Input_handler::Instance().Get_mouse_position();
  m_helicopter_middle_pos.x = ( m_position.x + ( m_size.x / 2 ));
  m_helicopter_middle_pos.y = ( m_position.y + ( m_size.y / 4 ));

  this->Hendle_input();
  this->Other_forces();
  m_position += (( m_velocity + m_velocity ) * (float)m_delta_time );

  m_frame_speed = 33.33f;
  if( m_acceleration_steering.y > 0 )  m_frame_speed = 50.0f;
  if( m_acceleration_steering.y < 0 )  m_frame_speed = 11.11f;
...
{
  if( ! (( m_helicopter_pitch_degrees > 30.0f )&&( m_helicopter_pitch_degrees < 330.0f )))
    {
      if(( m_velocity.x > 0 )&&( m_mouse_pos.x > m_helicopter_middle_pos.x ))  
	m_texture_flip = SDL_FLIP_NONE;
      if(( m_velocity.x < 0 )&&( m_mouse_pos.x < m_helicopter_middle_pos.x ))  
	m_texture_flip = SDL_FLIP_HORIZONTAL;
    }

  the_Texture_manager::Instance().Draw_frame_rot( the_Game::Instance().Get_renderer(), m_texture_id, m_position, m_size,
...
void Game_obj_sheet_player::Hendle_input()
{
  ...
  m_acceleration_steering.x = 0;
  m_acceleration_steering.y = 0;
  float steering_speed = ( 60.0d * m_delta_time );
...
{
  // THE GRAVITY:
  float m_gravity_forces = ( 9.8 * 2 * m_delta_time );
  glm::vec2 acceleration_gravity( 0, m_gravity_forces );
